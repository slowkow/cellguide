...
<nav class="navbar navbar-expand-lg navbar-light bg-light mb-5">
  ...
  function buildMarkerTables() {
    ...
    for (var clusterIndex = 0; clusterIndex < clusterNames.length; clusterIndex++) {
      ...
      for (let tabIdx = 0; tabIdx < tabInfo.length; tabIdx++) {
        ...
        htmls.push(`<div id="${divName}" style="height:500px;">`);
  ...
  function loadMarkersFromTsv2(papaResults, url, divId, clusterName, clusterIndex) {
    ...
    // $("#"+divId).html(`<table class='table' id='tpMarkerTable-${clusterIndex}'></table>`)
...
//  }
  ...
  function vertical_legend({
	...
	function mygene_query(gene, callback) {
		...
		request.open(
			'GET', 'https://mygene.info/v3/query?q=symbol:' + gene + '&species=human', true
		);
